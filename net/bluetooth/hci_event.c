				      ...
				      u8 reason)
{
	...
	hci_send_cmd(hdev, HCI_OP_LE_CONN_PARAM_REQ_NEG_REPLY,
		     sizeof(cp), &cp);
}
					     ...
					     struct sk_buff *skb)
{
	...
	mgmt_new_conn_param(hdev, &hcon->dst, hcon->dst_type, min, max, latency,
			    timeout);
					    ...
					    struct sk_buff *skb)
{
	...
	conn->le_interval = __le16_to_cpu(ev->interval);
	conn->le_latency = __le16_to_cpu(ev->latency);
	conn->le_supervision_timeo = __le16_to_cpu(ev->timeout);
}
