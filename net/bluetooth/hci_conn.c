...
{
	if (status)
		BT_ERR("Failed to disable LE scanning: status 0x%2.2x",
		       status);
}

static int remove_passive_scan_trigger(struct hci_dev *hdev)
{
	...
	err = hci_req_run(&req, disable_passive_scan_complete);
	if (err)
		return err;
